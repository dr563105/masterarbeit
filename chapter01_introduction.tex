...
a major obstacle exists in terms of cost associated with integrating all the necessary
sensors in addition to manufacturing a vehicle.
...
As briefly mentioned, the high cost of associated sensors such as
LIDAR\cite{mesarticleonLidar},  has put off many smaller research groups from using them
...
\section{Goal}
    ...
    \begin{enumerate}
    \item Building an autonomous driving framework which comprises of the following three
        components:
        \begin{itemize}
        \item ROS - use ROS2 to synchronise the data received from the simulator through a
            ...
            \textbf{(Ask
            where to write about sensor plugin work)}
