...
\section{Why is simulation needed?}
...
\section{What is in a simulator?}

Data collection is one of the most important phase in supervised learning. So caution must
be taken in choosing a simulator. A simulator must fulfil certain conditions to be
qualified as a good one.
\begin{itemize}
    \item It must have an vehicle that can move around in a virtual map.
...
\subsection{How is LGSVL simulator developed?}
...
3D environment also defines the rules how agents must behave such as stopping at traffic
...
comparing and contrasting correct and expect behaviours. A lot of variables like HD maps,
traffic movement behaviour and their density, time of the day, weather condtions etc. also
play a role while testing. It is also possible to write scripts with the help of Python
API, scenarios can be created and tested.
