...
\end{enumerate}
\section{Velocity}
Velocity is a scalar value, labelled output. Predicting velocity using convolutional
layers that extract features would be a tedious task. However, we test the neural network
with various configuration, like we accomplished in section \ref{chapter5sec:cce}, to
force the convolutional layers to extract features, LSTM to carry the temporal
information, and fully connected/dense layers to predict the velocity in addition to classification
...
\begin{figure}[!ht]
	\centering
    \def\svgwidth{\textwidth}
    \input{figures/inkscape/velocity1.pdf_tex} %use full path to know the location of pdftex
    \caption{Losses for NN architecture a) in figure \ref{fig:velocitycompareNN}}
    \label{fig:velocity1}
\end{figure}
\begin{figure}[!ht]
	\centering
    \def\svgwidth{\textwidth}
    \input{figures/inkscape/velocity2.pdf_tex} %use full path to know the location of pdftex
    \caption{Losses for NN architecture b) in figure \ref{fig:velocitycompareNN}}
    \label{fig:velocity2}
\end{figure}
\begin{figure}[!ht]
	\centering
    \def\svgwidth{\textwidth}
    \input{figures/inkscape/velocity3.pdf_tex} %use full path to know the location of pdftex
    \caption{Losses for NN architecture c) in figure \ref{fig:velocitycompareNN}}
    \label{fig:velocity3}
\end{figure}
