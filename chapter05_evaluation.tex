...
observe how light influences the prediction of output(see table \ref{table:timeoftheday}). Only steering angle is predicted
...
\begin{figure}[h]
    ...
    \caption{\textbf{Upper left}: Datasets vs Light Conditions vs Collisions.
        \textbf{Upper right}: Afternoon - Datasets vs Traffic vs Number of Collisions.
\textbf{Lower left}: Average number of collisions in percentage}
    \label{fig:dsvslcvstrafficAll}
\end{figure}

It is seen from figure \ref{fig:dsvslcvstrafficAll}(top left) that afternoon time provides the best light conditions for all the three
...
\ref{fig:dsvslcvstrafficAll}(bottom left), is calculated, dataset 3 performs the best among
...
From figure \ref{fig:dsvslcvstrafficAll}(top right), we can observe that all three datasets do
...
\section{Steering angle - Choosing the loss function}
Since steering angle is a continuous value ranging between -1 and 1(negative values to
turn left and positive values to turn right), a continuous loss function has to be used.
Because of that \textit{mean square error}(MSE) is chosen. And from figure
\ref{fig:dsvslcvstrafficAll}, we can observe that predicting only steering angle with MSE
as loss functions works.
...
that dataset 1 and the validation loss starts to overfit too quickly in the training(see
...
\begin{tabular}{lcc}
    ...
    Stops at random places & Yes & Yes \\
    Navigates traffic smoothly & No & No\\\bottomrule
\end{tabular}
...
acceleration values mean braking is active. With that thought in mind, sigmoid as activation function and mean
...
\begin{tabular}{lcc}
    ...
    Smooth steering control at turnings & No & No\\
    Stops at random places & Yes & Yes \\
    Detects vehicles as dynamic objects & Yes & Yes \\
    Navigates traffic smoothly & No & No\\\bottomrule
\end{tabular}
...
though performs well in 3 out of 8 conditions, shows bad steering behaviour at traffic or high
...
Unfortunately in our case, this balance is not achieved. So this limit in mind, the
...
In Keras, the LSTM units refers to the dimension of hidden state vector \textit{h} that is the state output from RNN cell.
...
the steering was all over the place and acceleration was not stable at all, resulting in many collisions.

The cause for this behaviour is investigated and it is found that since both
classification task outputs and steering output inherit the same dense layer 3, sufficient
learning is not observed as steering is perhaps overwhelmed while training and its weight
are not properly optimised. This paves way nicely to the next design iteration.
\begin{figure}[h]
	\centering
    \def\svgwidth{0.8\textwidth}
...
\begin{tabular}{lc}
    ...
    Stops at random places & Yes \\
    Detects vehicles as dynamic objects & Yes \\
    Navigates traffic smoothly & No\\\bottomrule
\end{tabular}
...
\begin{tabular}{lc}
    ...
    Stops at random places & Yes \\
    Detects vehicles as dynamic objects & Yes \\
    Navigates traffic smoothly & Yes\\\bottomrule
\end{tabular}
...
\begin{enumerate}
    \item Tuning the neural network albeit only the fully connected layers, results in
        ...
        outputs.
    \item Probably because of imbalance in the dataset, the car while stops at random
        positions, and struggles to recover from it.
    \item Separating into two NNs allows better steering control at a higher speed.
    \item Sunlight still plays a major role. Sunlight makes the model do
        random, strange crashes. Shadows however, seems like to not influence much the car
